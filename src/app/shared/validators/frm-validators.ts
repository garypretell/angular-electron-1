...
export function onlyNumber(): any {
  return (control: FormControl): ValidationErrors | null => !control.value || ONLY_NUMBER_REGEX.test(control.value) ? null : { onlyNumber: true };
}
export function phoneFormat(): any {
  return (control: FormControl): ValidationErrors | null => !control.value || PHONE_REGEX.test(control.value) ? null : { phoneFormat: true };
}
export function onlyNumberCustom(): any {
  return (control: FormControl): ValidationErrors | null => !control.value || ONLY_NUMBER_CUSTOM_REGEX.test(control.value) ? null : { onlyNumber: true };
}
export function onlyLetter(): any {
  return (control: FormControl): ValidationErrors | null => !control.value || ONLY_LETTER_REGEX.test(control.value) ? null : { onlyLetter: true };
}

export function alphanumeric(): any {
  return (control: FormControl): ValidationErrors | null => !control.value || ALPHANUMERIC_REGEX.test(control.value) ? null : { alphanumeric: true };
}
